...
class FastSpeech2Model(SpectrogramGenerator):
    ...
    def generate_spectrogram(self, tokens: torch.Tensor, speaker: torch.Tensor) -> torch.Tensor:
        self.eval()
        token_len = torch.tensor([tokens.shape[1]]).to(self.device)
        spect, *_ = self(text=tokens, text_length=token_len, speaker=speaker)
