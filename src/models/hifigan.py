    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            for i in range(audio.shape[0]):
                real_audio = audio[i, : audio_len[i]].data.cpu().numpy()
                generated_audio = audio_pred[i, 0, : audio_len[i]].data.cpu().numpy().astype("float32")
                denoised_audio = pred_denoised[i, : audio_len[i]].data.cpu().numpy()

                tb_logger.add_audio(
                    f"audio/real_audio_{i}", real_audio, self.global_step, sample_rate=self.sample_rate
                )
                tb_logger.add_audio(
                    f"audio/generated_audio_{i}", generated_audio, self.global_step, sample_rate=self.sample_rate
                )
                tb_logger.add_audio(
                    f"audio/denoised_audio_{i}", denoised_audio, self.global_step, sample_rate=self.sample_rate
                )

                input_mel = plot_spectrogram_to_numpy(audio_mel[i, :, : audio_mel_len[i]].data.cpu().numpy())
                output_mel = plot_spectrogram_to_numpy(audio_pred_mel[i, :, : audio_mel_len[i]].data.cpu().numpy())
                denoised_mel = plot_spectrogram_to_numpy(
                    pred_denoised_mel[i, :, : audio_mel_len[i]].data.cpu().numpy()
                )

                tb_logger.add_image(f"input_mel_{i}", input_mel, self.global_step, dataformats="HWC")
                tb_logger.add_image(f"output_mel_{i}", output_mel, self.global_step, dataformats="HWC")
                tb_logger.add_image(f"denoised_mel_{i}", denoised_mel, self.global_step, dataformats="HWC")

                if self.input_as_mel:
                    gt_mel = plot_spectrogram_to_numpy(gt_mel[i, :, : audio_mel_len[i]].data.cpu().numpy())
                    tb_logger.add_image(f"gt_mel_{i}", gt_mel, self.global_step, dataformats="HWC")
